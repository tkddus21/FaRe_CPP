...
### 계획 커버리지는 실제 커버리지가 아닙니다

위의 모든 수치는 **계획 시점** 값입니다. `report_coverage()`는 모든 waypoint에 정확히 도달해 정확한 방향을 봤다고 가정하고 FOV를 합집합하며, `trash_eval.py`도 docstring에 "purely geometric"이라고 적혀 있습니다. 25/26으로 끝난 실행은 전부 도달한 게 아닙니다. 이 비율을 goal 도달률 옆에 나란히 적으면 읽는 사람이 앞의 것을 뒤의 것으로 오해하기 쉽습니다.

```bash
python3 FaRe/achieved_coverage.py results/turtlebot3_house/<실행디렉터리>
python3 FaRe/achieved_coverage.py results/<실행디렉터리> --along-path   # 주행 중 훑은 것까지 포함
```

같은 모델(`Scout.cast_fov()`, 같은 `surveillance_range`, 같은 격자)을 그대로 쓰되, 각 goal이 끝난 시점에 로봇이 **실제로 있던 포즈**에서 캐스팅합니다. 포즈는 그 실행의 `patrol.bag`에서 tf(`map` → `odom` → `base_footprint`)로 읽습니다. `/amcl_pose`는 약 1.2 Hz라 최고속 주행 시 1 m까지 뒤처지므로 쓰지 않습니다. 결과로 `achieved_coverage.csv`(goal별 실제 포즈·위치 오차·방향 오차·새로 본 셀 수)와 `achieved_coverage.png`(초록 = 둘 다, 주황 = 계획만, 파랑 = 실제만, 빨강 = 둘 다 아님)를 남깁니다.

turtlebot3_house 실측, waypoint 집합은 같고 goal 도달률만 다른 경우:

| 실행 | goal | 계획 | **실제** | 위치 오차 중앙값 |
|---|---|---|---|---|
| 배치 수정 전 | 12/26 | 98.8% | **79.4%** | 0.115 m |
| 현재 | 25/26 | 98.1% | **97.9%** | 0.026 m |
| AWS, 현재 | 25/26 | 96.2% | **95.5%** | 0.024 m |

위 두 행에서 계획 수치는 거의 움직이지 않습니다 — 그건 waypoint를 설명하는 값이고 waypoint는 양쪽 다 멀쩡했기 때문입니다. 배치 수정이 실제로 산 것은 실제 커버리지 쪽에 있습니다: **79.4% → 97.9%**. goal 도달률이 대신 말하고 있던 수치가 이것입니다.

위치 오차 중앙값 자체도 유용한 점검입니다. 0.026 m는 DWA의 `xy_goal_tolerance` 0.05 m 안쪽이며, 제대로 도착했을 때의 모습입니다.
...
- **DWA가 sub-metre goal에서 목표로부터 멀어짐 (미해결):** 배치 수정 뒤에도 남는 실패는 지역 계획기가 **여유가 충분한 곳에서** 길을 잃는 것입니다. 기록된 bag으로 추적했습니다. `results/turtlebot3_house/20260811_1736_house_waffle_pi`에서 goal #8 (6.95, 0.10)이 120초 `TIMEOUT`했습니다. waypoint는 멀쩡했고(여유공간 0.350 m, 접근 경로 0.79 m 안에 벽 없음) 계획도 멀쩡했습니다 — NavFn의 첫 plan은 1.4 m 직선이었고 120초 내내 끝점이 목표에 붙어 있었습니다. 로봇은 후진으로 **0.193 m**까지 접근해 x를 6.51 → 6.78로 좁혔지만 y가 0.190에 멈춘 채 목표 0.10까지 못 갔고, 이후 전진으로 뒤집혀 동쪽을 가리키는 계획을 두고 **서쪽으로 12 m**를 달렸습니다. tf 샘플 3719개 중 목표 0.15 m 안에 들어온 것은 하나도 없습니다.
  ...
  아직 손대지 않은 것: 스톡 DWA 파라미터, 그리고 `set_goals()`가 애초에 근접 중복 waypoint를 만들어내는 습관. 무엇을 시도하든 조건마다 여러 번 돌려야 합니다 — 이 맵에서 단일 실행으로는 수정과 운을 구분할 수 없습니다.
